...
(function($, window, document, undefined) {
	...
	var PullRefresh = $.Scroll.extend($.extend({
		...
		endPullupToRefresh: function(finished,contentusernomore) {
			var self = this;
			if (self.bottomPocket) { // && self.loading && !this.pulldown
				self.loading = false;
				if (finished) {
					this.finished = true;
					self._setCaption(contentusernomore || self.options.up.contentnomore);
					//					self.bottomPocket.classList.remove(CLASS_VISIBILITY);
					//					self.bottomPocket.classList.add(CLASS_HIDDEN);
					self.wrapper.removeEventListener('scrollbottom', self);
				} else {
					self._setCaption(contentusernomore || self.options.up.contentdown);
					//					setTimeout(function() {
		...
		endPullupToRefresh: function(finished,contentusernomore) {
			...
			if (self.pullLoading) {
				...
				if (finished) {
					...
					self.pullCaption.innerHTML = contentusernomore || self.options.up.contentnomore;
